import React from "react";
import { Link } from "react-router-dom";

const NavBar = () => {
  return (
    <div
      className="
        bg-white 
        flex items-center justify-between 
        border border-blue-400 
        rounded-3xl shadow-lg 
        w-[97%] mx-auto mt-3 px-4 py-2
      "
    >
      {/* LEFT SIDE — LOGO + NAME */}
      <Link
        to="/user/homepage"
        className="flex items-center gap-2 cursor-pointer"
      >
        <img
          src="/image-removebg.png"
          alt="logo"
          className="w-10 h-14 object-contain"
        />
        <span className="text-2xl md:text-3xl font-bold text-black">
          recruito
        </span>
      </Link>

      {/* RIGHT SIDE NAV LINKS */}
      <div className="flex items-center gap-4">
        <div className="hidden sm:block text-orange-400 text-lg md:text-2xl font-semibold cursor-pointer">
            <a href="https://www.coursera.org/">Prepare</a>
          </div>
        <div className="hidden sm:block text-green-400 text-lg md:text-2xl font-semibold cursor-pointer">
            <a href="https://welovedevs.com/app/tests" >Practice</a>
          </div>

        {/* Profile Image */}
        <Link to="/user/editprofile" className="w-10 h-10 md:w-12 md:h-12 cursor-pointer">
          <img
            className="w-full h-full rounded-full object-cover"
            src="data:image/png;base64,"
            alt="user"
          />
        </Link>
      </div>
    </div>
  );
};

export default NavBar;
